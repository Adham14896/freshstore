...
export default function Register() {
  ...
  const { userId, setUserId, setUserData } = useContext(UserContext);
  ...
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      .min(3, "Your name must be 3 letters at least")
    ...
    phone: Yup.string()
      .matches(
        /^(002)?01[0125][0-9]{8}$/,
        "Invalid phone number, phone number must be an Egyptian number"
      )
      ...
      <div className="w-1/2 pt-8 mx-auto">
        ...
        <form onSubmit={formik.handleSubmit} className="py-10">
          ...
          {formik.errors.phone && formik.touched.phone && (
            <div
              className="p-4 mb-4 space-y-4 text-sm text-red-800  rounded-lg bg-red-50  dark:text-red-400"
              role="alert"
            >
              {formik.errors.phone}
            </div>
        ...
        <div className="text-center">
          <p className="text-gray-500 text-lg">Already have Account?</p>
          <Link className="text-green-500 text-base" to="/login">
            Login now
          </Link>
        </div>
      </div>
